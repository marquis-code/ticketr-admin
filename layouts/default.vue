  ...
  <div class="min-h-screen flex bg-[#f8fafc]">
    ...
    <aside :class="['fixed lg:static inset-y-0 left-0 z-50 bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out', isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full lg:translate-x-0', isSidebarCollapsed ? 'lg:w-20' : 'lg:w-64']">
      ...
      <nav class="flex-1 py-6 px-3 space-y-1.5 overflow-hidden">
        <NuxtLink to="/dashboard" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path === '/dashboard' }">
        ...
        <NuxtLink to="/dashboard/events" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.startsWith('/dashboard/events') }">
        ...
        <NuxtLink to="/dashboard/attendees" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path === '/dashboard/attendees' }">
        ...
        <NuxtLink to="/dashboard/orders" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/orders') }">
        ...
        <NuxtLink to="/dashboard/internal-tickets" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/internal-tickets') }">
          <CreditCard class="w-5 h-5 shrink-0" />
          <span :class="{'hidden lg:hidden': isSidebarCollapsed, 'block': !isSidebarCollapsed}" class="whitespace-nowrap">Internal Tickets</span>
        </NuxtLink>
        <NuxtLink to="/dashboard/scanner" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/scanner') }">
        ...
        <NuxtLink to="/dashboard/communications" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/communications') }">
        ...
        <NuxtLink to="/dashboard/settings" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/settings') }">
        ...
        <NuxtLink to="/dashboard/audit-logs" class="nav-link flex items-center gap-3" :class="{ 'nav-link-active': route.path.includes('/audit-logs') }">
